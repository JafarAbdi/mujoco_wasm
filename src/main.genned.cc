...
public:
  ...
  val mesh_texcoord() const {
    return val(typed_memory_view(m->nmesh * 1, m->mesh_texcoord));
  }
...
public:
  ...
  void solveM2(val x, val y, val sqrtInvD, int n) {
    return mj_solveM2(_model->ptr(), _state->ptr(),
                      reinterpret_cast<mjtNum *>(x["byteOffset"].as<int>()),
                      reinterpret_cast<mjtNum *>(y["byteOffset"].as<int>()), 
                      reinterpret_cast<mjtNum *>(sqrtInvD["byteOffset"].as<int>()), 
                      n);
  }
...
EMSCRIPTEN_BINDINGS(mujoco_wasm) {
  ...
  value_object<mjOption>("mjOption")
      ...
      //gravitational acceleration .field("wind"              , &mjOption::wind)
