...
sys.path.append(os.path.abspath(f"{mujoco_path}/introspect"))
...
def parse_pointer_line(
    ...
    if parse_mode[1] == "model":
        mj_definitions.append(
            "  val  "
            + elements[1].ljust(22)
            + " () const { return val(typed_memory_view(m->"
            + elements[2].ljust(15)
            + " * "
            + elements[3].ljust(9)
            + ", m->"
        ...
        mj_definitions.append(
            "  val  "
            + elements[1].ljust(22)
            + " () const { return val(typed_memory_view(_model->ptr()->"
            + elements[2].ljust(15)
            + " * "
            + elements[3].ljust(9)
            + ", _state->ptr()->"
    ...
    emscripten_bindings.append(
        "      .property("
        + ('"' + elements[1] + '"').ljust(24)
        + ", &"
        + ("Model" if parse_mode[1] == "model" else "Simulation")
        + "::"
        + elements[1].ljust(22)
        + ")"
    )
    ...
    for model_line in header_lines:
        if elements[0] + "* " in model_line and elements[1] + ";" in model_line:
            ...
            break
    typescript_definitions.append(
        "  "
        + elements[1].ljust(22)
        + ": "
        + types_to_array_types[elements[0]].rjust(12)
        + ";"
    )
...
def parse_int_line(
    ...
    typescript_definitions.append(
        "  " + name.ljust(22) + ": " + ("number").rjust(12) + ";"
    )


with open(f"{mujoco_path}/include/mujoco/mjmodel.h") as f:
    model_lines = f.readlines()

with open(f"{mujoco_path}/include/mujoco/mjdata.h") as f:
    data_lines = f.readlines()


# Parse mjx Macro to get the emscripten bindings and typescript definitions
with open(f"{mujoco_path}/include/mujoco/mjxmacro.h") as f:
    lines = f.readlines()
...
for function in functions.FUNCTIONS:
    ...
    valid_function = return_decl == "const char *" or (
        not ("*" in return_decl) and not ("[" in return_decl)
    )
    for param in functions.FUNCTIONS[function].parameters:
        ...
        if param.decltype == "const mjModel *":
            ...
            def_args.append(
                "val " + param.name
            )  # (str(param)) # UNTESTED, WE DON'T KNOW IF THIS WORKS
            ...
            def_params.append(
                "reinterpret_cast<mjtNum*>(" + param.name + '["byteOffset"].as<int>())'
            )  # .global("byteOffset").as<unsigned>()
            def_typescript.append(param.name + " : Float64Array")
            need_raw_pinters = True
        elif (
            not ("*" in param_type)
            and not ("[" in param_type)
            and not (param_type == "mjfPluginLibraryLoadCallback")
        ):
            def_args.append(str(param))
            def_params.append(param.name)
            param_type = (
                param_type.replace("mjtNum", "number")
                .replace("int", "number")
                .replace("float", "number")
                .replace("size_t", "number")
                .replace("unsigned char", "string")
            )
    ...
    if valid_function:
        ...
        auto_gen_lines["data_definitions"].append(
            "  "
            + return_decl.ljust(6)
            + " "
            + name.ljust(20)
            + "("
            + (", ".join(def_args)).ljust(20)
            + ") { return "
            + to_return
            + "); }"
        )
        auto_gen_lines["data_bindings"].append(
            "      .function("
            + ('"' + name + '"').ljust(23)
            + " , &Simulation::"
            + name.ljust(22)
            + (")" if not need_raw_pinters else ", allow_raw_pointers())")
        )  # <arg<mjtNum*>>
        auto_gen_lines["data_typescript"].append(
            "  /** "
            + functions.FUNCTIONS[function].doc
            + (
                "    [Only works with MuJoCo Allocated Arrays!]"
                if need_raw_pinters
                else ""
            )
            + "*/"
        ...
        returnType = (
            returnType.inner_type.name if "*" in returnType.decl() else returnType.name
        )
        returnType = (
            returnType.replace("mjtNum", "number")
            .replace("int", "number")
            .replace("float", "number")
            .replace("char", "string")
        )
...
for line in model_lines:
    ...
    if cur_enum_name is not None and len(line) > 0:
        ...
        if len(parts[0]) > 0 and len(parts[0].split(" ")) > 0:
            ...
            potatos = parts[1] if len(parts) >= 2 else ""
            auto_gen_lines["model_enums"].append(
                "      .value("
                + ('"' + meat + '"').ljust(25)
                + ", "
                + cur_enum_name.ljust(25)
                + "::"
                + meat.ljust(25)
                + ")"
            )
    ...
    if line.startswith("typedef enum"):
        ...
        auto_gen_lines["model_enums"].append(
            "  enum_<" + cur_enum_name + '>("' + cur_enum_name + '")'
        )
        if len(line.split("//")) > 1:
            auto_gen_lines["enums_typescript"].append(
                "/** " + line.split("//")[1].ljust(40) + " */"
            )
...
with open("src/main.template.cc") as f:
    ...
    with open("build/main.genned.cc", mode="w") as f:
        f.write(content)
